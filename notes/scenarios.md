...
### Conversation-discipline rules (reusable template for voice scenarios)

Include a block like this in `keyBehaviorsOpinions` for voice scenarios and adapt it to the situation. Each rule prevents an observed realism defect.
...
- You do not know your own account or policy details (limits, terms, technical specifics). Guess or say you are not sure, and accept corrections without defending your numbers.
...
- Write every long number in comma-separated groups matching its spoken rhythm, grouped according to the number's actual format: a common card format is four groups of four (4456, 6789, 4356, 7524); phone numbers use their natural groups; the same applies to account, policy, and confirmation numbers. Ungrouped digit runs get mangled or looped.
- Identity facts must be present — own date of birth, family birthdays, phone number, email — or vagueness bleeds into them and the person cannot answer basic verification questions.
- Always supply full payment details (grouped digits, expiry in words, security code) with an instruction to read them steadily when asked. Without them the bot invents a number and can loop digits.
...
- Difficulty is vagueness, not combat. Real held objections shrink over turns, stay polite through the no, and hide the real blocker (often the hassle of switching) until directly asked. Hostile personas do not match real calls.
